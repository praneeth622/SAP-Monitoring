...
const normalizeTemplate = (template: ApiTemplate): NormalizedTemplate => {
  ...
  const id = Array.isArray(template.template_id) ? template.template_id[0] : template.template_id;
  const name = Array.isArray(template.template_name) ? template.template_name[0] : template.template_name;
  const description = Array.isArray(template.template_desc) ? template.template_desc[0] : template.template_desc;
  const isDefault = Array.isArray(template.default) ? template.default[0] : template.default;
  const isFavorite = Array.isArray(template.favorite) ? template.favorite[0] : template.favorite;
  const frequency = template.frequency ? (Array.isArray(template.frequency) ? template.frequency[0] : template.frequency) : "auto";
  
  // Extract systems
  const systems = template.systems?.map(system => system.system_id) || [];

  // Extract and transform graphs
  const graphs = template.graphs?.map(graph => {
    // Parse position data
    ...
    const secondaryKpis = graph.secondary_kpis?.map((sk: { kpi_id: string }) => sk.kpi_id) || [];
...
const generateChartsFromTemplate = (template: NormalizedTemplate, resolution = 'auto') => {
  console.log(`Generating charts from template "${template.name}" with ${template.graphs?.length || 0} graphs`);
  ...
  template.graphs.forEach(graph => {
    ...
    if (graph.secondaryKpis) {
      graph.secondaryKpis.forEach(kpi => allTemplateKpis.add(kpi.toLowerCase()));
    }
  });
  ...
  console.log(`Assigned colors to ${Object.keys(globalKpiColors).length} unique KPIs`);

  return template.graphs.map((graph, index) => {
    // Generate data for all KPIs in the graph
    const allKpis = [graph.primaryKpi, ...graph.secondaryKpis].map(kpi => kpi.toLowerCase());
    ...
    const valueRanges: Record<string, { base: number, amplitude: number, trend: number }> = {};
    ...
    for (let i = 0; i < dataPoints; i++) {
      ...
      allKpis.forEach((kpi) => {
        ...
        if (kpi.includes('cpu')) {
          ...
          pattern = Math.sin(i / 4) * amplitude/2 + (Math.random() * amplitude/2);
    ...
    console.log(`Generated ${dummyData.length} dummy data points for chart "${graph.name}"`);
...
export default function Dashboard() {
  ...
  const fetchTemplates = async () => {
    try {
      ...
      if (defaultTemplate) {
        ...
        const templateCharts = generateChartsFromTemplate(defaultTemplate, resolution);
        
        if (templateCharts.length > 0) {
          console.log(`Generated ${templateCharts.length} charts from default template`);
        ...
        console.log(`No default template found, using first template: "${firstTemplate.name}"`);
        setSelectedApiTemplate(firstTemplate.id);
        
        // Generate charts from the first template
        const templateCharts = generateChartsFromTemplate(firstTemplate, resolution);
        
        if (templateCharts.length > 0) {
          console.log(`Generated ${templateCharts.length} charts from first template`);
          setCharts(templateCharts);
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      try {
        ...
        if (!isMounted) return;
        console.log("Templates loaded, API templates count:", apiTemplates.length);
  ...
  const selectedCharts = selectedApiTemplate
    ? charts
    : charts.filter((_, index) => templateData[selectedTemplate as TemplateKey].charts.includes(index));
      ...
      <main className={`flex-1 overflow-y-auto transition-all duration-300`}>
        <div className="container mx-auto px-8 py-6">
          <div className="flex flex-col mb-8">
            ...
            >
              {/* Template Controls - Row 1 */}
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">Default Layout</label>
              ...
              {/* User Templates - Row 1 */}
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">User Templates</label>
                <Select 
                ...
                >
                  <SelectTrigger className="w-full h-10">
                    <SelectValue placeholder="Select user template" />
                  </SelectTrigger>
                  <SelectContent>
              ...
              {/* Theme Selection - Row 1 */}
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">Chart Theme</label>
            ...
            {/* Date Range in its own row - Row 2 */}
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              className="mb-4"
